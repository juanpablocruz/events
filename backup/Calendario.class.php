...
class Calendario {
	...
	function draw_calendar($mes, $ano){ //si $mes es 0, se toma como el actual mes
		...
		$calendar.= "<tr>";
		while($blank > 0){
			$calendar.= "<td class='calendario'></td>";
			$blank--;
		...
		while($contador_dias <= $dias_en_mes){
			if($contador_dias == $dia)
				$calendar.= "<td class='calendarioF calendario' onclick=alert('$contador_dias/$nombre_mes/$ano')> $contador_dias </td>";
			else
				$calendar.= "<td class='calendario' onclick=alert('$contador_dias/$nombre_mes/$ano')> $contador_dias </td>";
			$contador_dias++;
			$contador_dias_misma_semana++;
